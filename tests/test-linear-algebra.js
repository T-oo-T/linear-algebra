...
const { det, is_identity, is_square, is_invertable, matmul, transpose, dot, cross, swap_rows, scale_row, sum_rows, rref, inv, I, matrixConcat } = require("../src/linear-algebra")
...
test("cross product", () => {
    ...
    assert.deepEqual(cross([0,1,0],[0,0,1]), [1,-0,0]) // -0 and 0 have different bit presentations and Object.is(-0, 0) => false
    assert.throws(() => {
        cross([1,2,3],[3,4])
...
test("sum_rows", () => {
    ...
    assert.deepStrictEqual(sum_rows(A(), 1, 0, -4), [
        [1,2,3],
        [0,-3,-6],
        [7,8,9]
    ])
...
test("I", () => {
    assert.deepStrictEqual(I(1), [[1]])
    assert.deepStrictEqual(I(2), [[1,0], [0,1]])
    assert.deepStrictEqual(I(3), [
        [1,0,0],
        [0,1,0],
        [0,0,1]
    ])
...
test("matrixConcat", () => {
    assert.deepStrictEqual(matrixConcat([
        [1,2,3],
        [4,5,6],
        [7,8,9]
    ], [[7],[7],[7]]),
